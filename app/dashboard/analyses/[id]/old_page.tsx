  ...
  return (
    <div className="space-y-2 mt-3">
      {hardFlags?.map((f, i) => (
        <div
          key={i}
          className="text-sm rounded-md border border-red-200 bg-red-50 px-3 py-1.5 dark:border-red-800 dark:bg-red-950"
        >
          {f}
        </div>
      ))}
      {softFlags?.map((f, i) => (
        <div
          key={i}
          className="text-sm rounded-md border border-yellow-200 bg-yellow-50 px-3 py-1.5 dark:border-yellow-800 dark:bg-yellow-950"
        >
          {f}
        </div>
      ))}
    ...
    <div className="max-w-5xl space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          {analysis.safety_fit && (
            <span
              className={`text-sm font-medium ${fitColor[analysis.safety_fit] ?? ""}`}
            >
          ...
          {analysis.growth_fit && (
            <span
              className={`text-sm font-medium ${fitColor[analysis.growth_fit] ?? ""}`}
            >
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              <span
                className={`text-2xl font-bold ${analysis.aiden_score ? scoreColor(analysis.aiden_score) : ""}`}
              >
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              <span
                className={`text-2xl font-bold ${analysis.lexa_score ? scoreColor(analysis.lexa_score) : ""}`}
              >
                {analysis.lexa_score ?? "—"}
              </span>
            </div>
